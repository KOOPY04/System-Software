		...
		else
		{
			...
			for(line_count = 1 ; (c = process_line(&line)) != LINE_EOF; line_count++)
			{
				...
				else
                {
					...
					{
						if(line.fmt == FMT4)
							printf("%06X  %-8s +%-8s%-s", LOCCTR, line.symbol, line.op, line.operand1);
					...
					{
						if(line.fmt == FMT4)
							printf("%06X  %-8s +%-8s%-s", LOCCTR, "", line.op, line.operand1);
						else if(line.addressing == ADDR_IMMEDIATE)
							printf("%06X  %-8s %-8s #%-s", LOCCTR, "", line.op, line.operand1);
						else if(line.addressing == ADDR_INDIRECT)
							printf("%06X  %-8s %-8s @%-s", LOCCTR, "", line.op, line.operand1);
					...
					if (strlen(line.symbol) > 0)
                    {
						if(strcmp(line.op, "BYTE") == 0 || strcmp(line.op, "WORD") == 0 || strcmp(line.op, "RESB") == 0 || strcmp(line.op, "RESW") == 0)
						{
							symbol_flag = 1;
						}
						if(symbol_flag == 0)
						{
							strcpy(SYMTAB[symtab_index].symbol, line.symbol);
							SYMTAB[symtab_index].address = LOCCTR;
							symtab_index++;
						}
						symbol_flag = 0;
                    }
					...
					if(strcmp(line.op, "BYTE") == 0)
					{
						...
						{
							int byte_count = strlen(line.operand1) - 3;
							LOCCTR += byte_count;
						} 
						else if(line.operand1[0] == 'X') 
						{
							int byte_count = (strlen(line.operand1) - 3 + 1) / 2;
							LOCCTR += byte_count;
						}
					}
			...
			int program_length = LOCCTR;
            printf("\n\nProgram Length: %x\n\n\n", program_length);

			printf("SYMBOL TABLE¡G\n");
